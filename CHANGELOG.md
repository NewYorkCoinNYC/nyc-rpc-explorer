...
* New minor misc peer data available in NewYorkCoin Core RPC v0.21+
...
* New data points in homepage "Network Summary":
	...
	* New data based on UTXO-set summary. Note that UTXO-set querying is resource intensive and therefore disabled by default to protect slower nodes. Set `NYCEXP_SLOW_DEVICE_MODE` to `false` in your `.env` file to enjoy associated features:
		* UTXO-set size
...
* Block Detail page improvements
	* New data in "Summary" on Block pages (supported for newyorkcoind v0.17.0+)
		* Outputs total volume
		* Input / Output counts
		* UTXO count change
		* Min / Max tx sizes
	* New "Fees Summary" section (newyorkcoind v0.17.0+)
...
* Homepage UI tweaks
	* Remove "NewYorkCoin Explorer" H1 (it's redundant)
...
* Fixes related to running newyorkcoind 0.19.0.1
