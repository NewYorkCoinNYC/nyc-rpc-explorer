...
const configPaths = [ path.join(os.homedir(), '.config', 'nyc-rpc-explorer.env'), path.join(process.cwd(), '.env') ];
...
debug.enable(process.env.DEBUG || "nycexp:app,nycexp:error");

const debugLog = debug("nycexp:app");
const debugErrorLog = debug("nycexp:error");
const debugPerfLog = debug("nycexp:actionPerformace");
...
if (process.env.NYCEXP_BASIC_AUTH_PASSWORD) {
	// basic http authentication
	expressApp.use(auth(process.env.NYCEXP_BASIC_AUTH_PASSWORD));

} else if (process.env.NYCEXP_SSO_TOKEN_FILE) {
	// sso authentication
	expressApp.use(sso(process.env.NYCEXP_SSO_TOKEN_FILE, process.env.NYCEXP_SSO_LOGIN_REDIRECT_URL));
}
...
function getSourcecodeProjectMetadata() {
	var options = {
		url: "https://api.github.com/repos/janoside/nyc-rpc-explorer",
		headers: {
			'User-Agent': 'request'
		}
	};
...
function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo, getindexinfo) {
	...
	var newyorkcoinCoreVersionRegex = /^.*\/Satoshi\:(.*)\/.*$/;

	var match = newyorkcoinCoreVersionRegex.exec(getnetworkinfo.subversion);
	if (match) {
		global.nycNodeVersion = match[1];

		var semver4PartRegex = /^([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)$/;

		var semver4PartMatch = semver4PartRegex.exec(global.nycNodeVersion);
		if (semver4PartMatch) {
			...
			global.nycNodeSemver = `${p0}.${p1}.${p2}`;

		} else {
			var semver3PartRegex = /^([0-9]+)\.([0-9]+)\.([0-9]+)$/;

			var semver3PartMatch = semver3PartRegex.exec(global.nycNodeVersion);
			if (semver3PartMatch) {
				...
				global.nycNodeSemver = "1000.1000.0"
			}
		}
	} else {
		// short-circuit: force all RPC calls to pass their version checks - this will likely lead to errors / instability / unexpected results
		...
		debugErrorLog(`Unable to parse node version string: ${getnetworkinfo.subversion} - RPC versioning will likely be unreliable. Is your node a version of NewYorkCoin Core?`);
	}
	
	debugLog(`RPC Connected: version=${getnetworkinfo.version} subversion=${getnetworkinfo.subversion}, parsedVersion(used for RPC versioning)=${global.nycNodeSemver}, protocolversion=${getnetworkinfo.protocolversion}, chain=${getblockchaininfo.chain}, services=${services}`);
	...
	coreApi.getBlockchainInfo().then(function(result) {
		...
		Promise.all(promises).then(function(results) {
			...
			if (results && results.length > 0 && results[0] != null) {
				...
				debugLog("Unable to load network volume, likely due to newyorkcoind version older than 0.17.0 (the first version to support getblockstats).");
			}
		});
	});
}
...
expressApp.continueStartup = function() {
	...
	global.rpcClient = new newyorkcoinCore(rpcClientProperties);
	...
	global.rpcClientNoTimeout = new newyorkcoinCore(rpcClientNoTimeoutProperties);
	...
	if (config.addressApi) {
		...
		if (!supportedAddressApis.includes(config.addressApi)) {
			utils.logError("32907ghsd0ge", `Unrecognized value for NYCEXP_ADDRESS_API: '${config.addressApi}'. Valid options are: ${supportedAddressApis}`);
		}
		...
		if (config.addressApi == "electrumx") {
			if (config.electrumXServers && config.electrumXServers.length > 0) {
				...
				utils.logError("327hs0gde", "You must set the 'NYCEXP_ELECTRUMX_SERVERS' environment variable when NYCEXP_ADDRESS_API=electrumx.");
			}
		}
	}
