...
const debugLog = debug("nycexp:rpc");
...
function getBlockStats(hash) {
	if (semver.gte(global.nycNodeSemver, minRpcVersions.getblockstats)) {
...
async function noTxIndexTransactionLookup(txid) {
	...
	// This is only available in Electrs and requires enabling NYCEXP_ELECTRUM_TXINDEX.
