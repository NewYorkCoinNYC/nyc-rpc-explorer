...
var debugLog = debug("nycexp:router");
...
var newyorkcoinCore = require("newyorkcoin-core");
var qrcode = require('qrcode');
var newyorkcoinjs = require('newyorkcoinjs-lib');
...
router.post("/connect", function(req, res, next) {
	...
	var newClient = new newyorkcoinCore({
		host: host,
		port: port,
		username: username,
		password: password,
...
router.get("/block-stats", function(req, res, next) {
	if (semver.lt(global.nycNodeSemver, rpcApi.minRpcVersions.getblockstats)) {
...
router.get("/address/:address", function(req, res, next) {
	...
	try {
		res.locals.addressObj = newyorkcoinjs.address.fromBase58Check(address);
	...
	try {
		res.locals.addressObj = newyorkcoinjs.address.fromBech32(address);
...
router.get("/rpc-terminal", function(req, res, next) {
	if (!config.demoSite && !req.authenticated) {
		res.send("RPC Terminal / Browser require authentication. Set an authentication password via the 'NYCEXP_BASIC_AUTH_PASSWORD' environment variable (see .env-sample file for more info).");
...
router.post("/terminal", function(req, res, next) {
	...
	if (cmd == "parsescript") {
		const nbs = require('node-newyorkcoin-script');
...
router.get("/newyorkcoin-whitepaper", function(req, res, next) {
	res.render("newyorkcoin-whitepaper");

	next();
});

router.get("/newyorkcoin.pdf", function(req, res, next) {
	// ref: https://newyorkcoin.stackexchange.com/questions/35959/how-is-the-whitepaper-decoded-from-the-blockchain-tx-with-1000x-m-of-n-multisi
